...
def get_material_presence_prediction(material_profile):
    """
    Generate prediction based on material size profile.
    Returns "1" if any size category is non-zero, "0" if all are zero.
    
    Args:
        material_profile (dict): Material profile containing size information
        
    Returns:
        str: "0" or "1"
    """
    if not material_profile or 'size' not in material_profile:
        return "0"
    
    size_data = material_profile['size']
    
    # Check if any size category has a non-zero value
    for size_category, value in size_data.items():
        if value > 0:
            return "1"
...
def process_raw_json_and_download():
    """
    Process raw.json file and create clean.json with downloaded images for Material Presence data.
    """
    print("Processing Material Presence raw.json and downloading images...")
    ...
    # Process each item, using 1-based IDs
    for idx, item in enumerate(items_to_process):
        i = idx + 1  # Start IDs from 1
        print(f"\nProcessing item {idx+1}/{len(items_to_process)} -> ID: {i}")
        
        # Extract original_image URL from images.originalImage
        original_image_url = ""
        if 'images' in item and 'originalImage' in item['images']:
            original_image_url = item['images']['originalImage']
        
        if not original_image_url:
            print(f"Warning: No original image found for item {i}")
        ...
        # Generate prediction based on material size profile
        predicted = get_material_presence_prediction(item.get('materialProfile', {}))
        ...
        if downloaded_path:
            print(f"✓ Successfully processed item {i} (predicted: {predicted})")
        else:
    ...
    try:
        ...
        predicted_ones = sum(1 for item in clean_data if item['predicted'] == '1')
        predicted_zeros = sum(1 for item in clean_data if item['predicted'] == '0')
        successful_downloads = sum(1 for item in clean_data if item['download_image'])
        
        print(f"\nSummary:")
        print(f"Total items: {len(clean_data)}")
        print(f"Successful downloads: {successful_downloads}")
        print(f"Predicted '1' (material present): {predicted_ones}")
        print(f"Predicted '0' (no material): {predicted_zeros}")
...
def main():
    ...
    """
    print("Material Presence Data Processing Script")
    print("=" * 45)
