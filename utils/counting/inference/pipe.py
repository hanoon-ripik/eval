...
from utils.dip_utils import cam_to_ccm_mapping, get_ist_timestamp, get_shift
...
class DIP:
    def __init__(self, config: Dict, clientId: str, produce: str) -> None:
        ...
        self.LINE_START: sv.Point = sv.Point(*config['ds-info']['line-start'])
        self.LINE_END: sv.Point = sv.Point(*config['ds-info']['line-end'])
        ...
        self.dia_handler: DiameterHandler = DiameterHandler(
            config['ds-info']['dia-handler'],
            config['ds-info']['ratios'],
            config['ds-info']['possible_dias']
        )
    ...
    def process(self) -> None:
        ...
        while True:
            ...
            frame = mask_annotator.annotate(scene=frame, detections=detections, opacity=0.5)
            ...
            if len(detections):
                ...
                for i in range(len(detections)):
                    ...
                    ret, pipe_id, diaMM = self.pipe_counter.process(
                        foc_tracker_id, box_corner_x1, box_corner_x2, mask_copy, frame, self.dia_handler
                    )

                    if ret and pipe_id not in self.this_shift_ids:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, help='Path to the config file for the camera.', required=True)
    parser.add_argument('--clientId', type=str, default='esldip-local', help='client id')
    parser.add_argument('--produce', type=str, default='debug', help='produce to debug')
    ...
    try:
        ...
        logger.error(f'Could not find config file {cfg_file}.')
        sys.exit()
