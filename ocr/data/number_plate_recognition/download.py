...
def process_raw_json_and_download():
    """
    Process raw.json file and create clean.json with downloaded images for Number Plate Recognition data.
    """
    print("Processing Number Plate Recognition raw.json and downloading images...")
    ...
    for idx, item in enumerate(items_to_process):
        ...
        # Extract original_image URL from originalImage (find cam_bay*_front)
        original_image_url = ""
        if 'originalImage' in item:
            # Look for cam_bay*_front cameras
            for camera_key in item['originalImage']:
                if camera_key.startswith('cam_bay') and '_front' in camera_key:
                    original_image_url = item['originalImage'][camera_key]
                    break
        ...
        # Extract OCR predicted value from vehicleNo
        ocr_predicted = ""
        if 'vehicleNo' in item:
            ocr_predicted = str(item['vehicleNo']).strip()
...
def main():
    ...
    """
    print("Number Plate Recognition Data Processing Script")
    print("=" * 40)
