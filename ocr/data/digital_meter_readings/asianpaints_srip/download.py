...
def process_raw_json_and_download():
    """
    Process raw.json file and create clean.json with downloaded images for Asian Paints SRIP data.
    """
    print("Processing Asian Paints SRIP raw.json and downloading images...")
    ...
    for i, item in enumerate(items_to_process):
        print(f"\nProcessing item {i+1}/{len(items_to_process)}")
        
        # Extract original_image URL from originalImage.cam_weighment_meter
        original_image_url = ""
        if 'originalImage' in item and 'cam_weighment_meter' in item['originalImage']:
            original_image_url = item['originalImage']['cam_weighment_meter']
        
        if not original_image_url:
            print(f"Warning: No cam_weighment_meter image found for item {i}")
        ...
        # Extract OCR predicted value from events.rmWeighed[0].weight
        ocr_predicted = ""
        if ('events' in item and 
            'rmWeighed' in item['events'] and 
            item['events']['rmWeighed'] and 
            len(item['events']['rmWeighed']) > 0 and
            'weight' in item['events']['rmWeighed'][0]):
            ocr_predicted = str(item['events']['rmWeighed'][0]['weight'])
...
def main():
    ...
    """
    print("Asian Paints SRIP Data Processing Script")
    print("=" * 40)
